...
*The variables vd1 and vd2  are needed for the splitting below (lines 38 and 63).
*Copying them keeps  the original values safe, as the splitting modifies them for each pseudo-observation.
...
*Declare as survival data (with fake failure variable) and time between jabs.
...
*The above stsplit (line 76) created spurious pseudo-observations, starting and ending on the same day. The next line drops them:
drop if after_primary ==1 & ibv ==1 & cohort_final ==1 & d1==-1 & ar==time & Primary_inf ==-1

*Line 88 labels the pseudo observation (line 88) where there is no previous primary infection with value Primary_inf=0.
*"Primary_inf" is currently -1 for the pseudo-observation before Primary infection. The value 0 is more logical.
*Also other values of Primary_inf are affected, but they are recoded below (line 91).
...
drop if x==21 //Comment this line for calculation of exposures for this category as well.

			
*Show table by with exposure, number of infections and crude rates (by vaccine Status)
...
*Fit the model
*This is slow with Efron method, remove "efron" option to use default Breslow method which is quicker.
stcox i.x i.Primary_inf i.region i.AGEGR2 i.GENDER i.ETHNIC_GR ib5.WORK_EXPOSURE_FREQUENCY i.occ_set_cat, vce(cluster Trust_Code) nolog efron
