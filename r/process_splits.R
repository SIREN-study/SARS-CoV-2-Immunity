...
split_ve %>% filter(eligible==1) %>% select(vaccine_cat) %>% table(useNA="ifany")
...
# check categories correctly assigned
split_dura %>% filter(eligible==1) %>% select(primary_inf) %>% table(useNA="ifany")
